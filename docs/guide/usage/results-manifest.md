...
`docs/traces/` 保存审核过的固定三例紧凑证据，包含动作摘要、terminal、最终 pose、worker 复用与
源 Manifest hash。它适合提交 Git。完整 Manifest 可能包含大量审计和轨迹，留在 `runs/`。
两者角色不同：trace 是受控证据，Manifest 是原始运行记录。
